...
export default function OnboardingView({ onJobCreated }: Props) {
      ...
      <div className="w-full max-w-xl">
        <header className="mb-10 text-center">
          ...
          <h1 className="mt-3 text-3xl font-semibold tracking-tight text-ink">
            Capturez votre prochaine réunion
          </h1>
          <p className="mt-3 text-sm text-ink-muted">
            Enregistrez en direct, importez un fichier audio, ou partez d&apos;un transcript déjà produit. Vous renseignerez le contexte juste après.
          </p>
        </header>
        ...
        <div className="rounded-2xl border border-surface-border bg-surface-card/80 p-8 shadow-soft backdrop-blur-sm">
          ...
          {mode === "transcript" && <TranscriptUploader onJobCreated={onJobCreated} />}
        </div>

        <p className="mt-6 text-center text-xs text-ink-muted">
          Les réunions sont enregistrées dans <span className="font-medium text-ink">Documents/Réunions</span>.
        </p>
      </div>
    </div>
  );
